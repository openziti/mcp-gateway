...
- **Ref-counted dialer attachments (for detach safety).** `Attach` is idempotent controller-side, so several backends naming the same tunnel already share one attachment. The dial abstraction ref-counts in-process holders so it detaches only when the *last* one closes — `Detach` removes the single shared record for everyone, so a premature detach would strand sibling backends. A real `ConnectTunnelConflict` (ambiguous name / UDP) is surfaced as an error, not swallowed.
...
- **Catalog publish unchanged except the mode constant**: the `catalog.PublishSpec` build now sets `TunnelMode: catalog.TunnelHTTP` (was `catalog.TunnelMode(identity.TunnelMode)`). Capability derivation, workgroup/contract merge, retract-on-shutdown all unchanged.
- **`Close`** order: retract advertisement → close `Serve` listener (delete tunnel iff managed) → `Dialer.Close()` (detach all) → close Agent. Continue on error, log each failure (preserve today's `withCleanupContext` pattern).
...
type Dialer struct {
    sub   *Subsystem
    mu    sync.Mutex
    attts map[string]*attachment // keyed by tunnel name; refs set of holders
}

func (s *Subsystem) Dialer() *Dialer
func (d *Dialer) HTTPClient(ctx context.Context, tunnel string) (*http.Client, error)
func (d *Dialer) Release(tunnel string)   // drop a holder ref; detach when last
func (d *Dialer) Close() error            // detach all
```

`HTTPClient` ensures the tunnel is attached (`Attach` is controller-idempotent, so a redundant call returns the existing attachment — no special conflict handling; surface a real `ConnectTunnelConflict` as an error) and bumps the holder ref-count, then returns an `*http.Client` whose `Transport.DialContext` ignores `addr` and returns `tunnel.Dial(ctx, agent, name)` — exactly parallel to `tools/access.go:56` (`sdk.NewDialer(token, root)`). Attach is process-level (one per unique name), **not** per session; per-connection work is the `Dial` inside `DialContext`. `Release`/`Close` detach a name only when its ref-count reaches zero (detach is not ref-counted controller-side).
...
- **`gateway/backend.go`** — replace the agora loopback fields/flow:
  ...
  - Inject the dial seam (Track D-seam) into discovery and the session factory in place of the resolver.
  - `Stop()`: `serve.Close(ctx)` (delete-iff-managed) + `dialer.Close()` replace the listener-close/server-shutdown at `backend.go:345-357`.
- **`gateway/session.go`** — rewrite `connectAgoraBackend` (`session.go:214`): drop the `cs.resolver(cfg.ID)` lookup (`session.go:218`); get `httpClient := dialer.HTTPClient(ctx, cfg.Transport.AgoraTunnel)` and build the SSE transport with it (host is a dummy; `DialContext` routes), mirroring `connectZrokBackend` (`session.go:173`). Remove the `resolver` field (`session.go:59,79`).
...
- **`agora/client.go`** — the dial-only `Client` no longer returns a loopback address. Rework `Dial(ctx, service)` to return an `*http.Client` (or expose the Subsystem's `Dialer`); `Close` detaches. Drop `dialTargetKey`/`ConnectTarget` usage.
...
- **`agora/identity.go`** — remove `TunnelMode` from `Identity`, `Defaults` (`TunnelMode`, `AllowedTunnelModes`), and `resolveIdentity`; delete `modeAllowed`.
...
- **`agora/subsystem_test.go`** — remove `BootstrapConnects`/loopback/`stubConnectAddress` tests; add create-or-bind tests (Listen-hits-existing vs ErrNotFound→Create→Listen, managed flag), wrong-mode-bind error, `Close` order, publish-mode-is-`http`, against the reshaped `agoraOps` mock.
- **New `agora/serve_test.go` / `agora/dial_test.go`** — Serve managed/bound paths and wrong-mode error; Dialer ref-counting (two names→two attaches; same name→one attach + ref-count 2; detach only on last release), and a real `ConnectTunnelConflict` surfaced as an error.
- **`agora/client_test.go`** — rewrite for the new `Dial` return shape.
...
- `agora/dial.go` — **new**, ref-counted Attach + `DialContext` http client (Track C)
...
## Open questions (decide during mercurius review)

1. **Dial seam type.** A bare `func(ctx, tunnel) (*http.Client, error)` injected into the aggregator (smallest delta, preserves decoupling) vs. a named interface. Recommend the func, mirroring today's injected `ConnectResolver`.
